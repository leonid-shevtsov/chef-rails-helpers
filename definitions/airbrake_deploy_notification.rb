define :airbrake_deploy_notification, {
  ...
  :host => nil,
    ...
    bash "airbrake deploy notification for #{name}" do
      code %Q{cd #{path} && curl -d "api_key=#{key}&deploy[username]=chef&deploy[rails_env]=#{environment}&deploy[scm_revision]=`git rev-parse HEAD`&deploy[scm_repository]=`git config --get remote.origin.url`" http://#{host}/deploys.txt}
    end
end